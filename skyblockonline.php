...
$discord->on('ready', function ($discord) {
    ...
    $discord->on('message', function ($message, $discord) {
        /*if(!empty($message->channel->guild->name) ){
            $message->reply("plz don't pm me. I'm not able to see those pm's");
            return;
        }*/
        ...
        if($message->content == "/ping"){
            ...
            // $message->channel->sendMessage("commands are disabled at the moment");
        }
        ...
        if(strpos($message->content, 'ddos') !== false){
            $message->reply("nope not on discord");
        }
        ...
        /*if($message->content == "/playmeasong"){
            ...
            $discord->joinVoiceChannel($authorChannel, false, false, null)->then(function (VoiceClient $vc)  {
                $vc->setVolume(10);
        ...
        if($message->content == "/help"){
            ...
            // $message->channel->sendMessage("/trigger => use when you are triggered");
			      // $message->channel->sendMessage("/mineh => HERE IS MINEH");
            // $message->channel->sendMessage("commands are disabled at the moment");
        }
    ...
    $game  = $discord->factory(Game::class, [
		...
		// 'name' => 'skyblock.online',
	]);
	$discord->updatePresence($game, true);
});
